    ...
    def get_likelihood(self):
        ...
        for i in self.label_index:
            ...
            for j in range(self.data.shape[1]):
                cnt = self.smoothing                #Laplace Smoothing
                for idx in self.label_index[i]:     #Count all the existing words
                    cnt += self.data[idx][j]
                self.likelihood[i].append(cnt)
            
            self.likelihood[i] = np.array(self.likelihood[i])
            self.likelihood[i] = self.likelihood[i]/denominator+self.epsilon        #Laplace Smoothing
    ...
    def get_posterior(self, x):
        ...
        for testcase in x:
            ...
            for lbl_idx in self.label_index:
                log_prior = np.log(self.prior[lbl_idx])       #log-scaled prior
                log_likelihood = np.log(1)
                for idx in range(testcase.size):
                    for i in range(testcase[idx]):
                        log_likelihood+=np.log(self.likelihood[lbl_idx][idx])      #calculate log-scaled likelihood
                pst = np.exp(log_prior+log_likelihood)
                tmp.append(pst)
